...
// gamma.go — Gamma Essence: personality extraction from embed_tokens
//
// γ lives in embed_tokens — the embedding layer carries identity.
// Stored as sparse NPZ: only tokens where finetune changed embeddings.
//
// Format (same as sparse delta):
...
// GammaEssence holds the personality diff for embed_tokens
type GammaEssence struct {
	...
	Values    []float32 // [NumTokens × EmbedDim] — f32 (only if source was f32)
