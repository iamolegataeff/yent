...
## Состояние (тул-проверено 2026-07-13)

Ветка `claude/metajanus`, **6 коммитов над origin/main**, дерево чистое, **build 0 ворнингов, 23/23
теста зелёные**. Слой ИЗМЕРИТЕЛЬНЫЙ и ИНЕРТНЫЙ — инференс эти 4 поля нигде не читает, генерацию не трогает.
Аудит Fable (F1–F6) разобран: F1 (исток недвижим через LOAD), F2 (soma мерж-безопасен), F3 (еврейское
лицо выведено, не зашито) — закрыты и залочены тестами; F4 инертность подтверждена; F5/F6 — заметка/коммент.
